...
///
/// The recipe is [`crate::paths::write_new_private`], shared with the session
/// fallback in `secrets` -- this copy is where the drift happened that the
/// shared function's doc records (`create(true).truncate(true)`, which opens
/// an existing file with whatever permissions it already had and follows a
/// symlink). `create_private_dir` chmods the directory to 0700 but removes
/// nothing already inside it, so a link planted while it was lax outlives the
/// tightening; `create_new` is what refuses it.
fn write_private(path: &Path, contents: &str) -> Result<(), ConfigError> {
    crate::paths::write_new_private(path, contents.as_bytes()).map_err(|source| {
        ConfigError::Write {
            path: path.to_path_buf(),
            source,
        }
