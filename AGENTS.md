...
| A story already on disk is not fetched again, and a story that stopped arriving is never left under its real name | `commands::stories::save_story` — the four extensions `extension_of` can answer are looked for before the request, and a hit costs nothing; the bytes then stream through `IgClient::download_to` into `<name>.part`, renamed into place only once the file is whole. A `.part` left by a killed process is removed the next time that story is asked for. The look is an optimization in front of `output::create_new`, not a replacement for it: a second `-d all` used to download every story in the tray and then refuse every name |
